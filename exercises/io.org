...
You may use that
  ...
  1. the function ~readFile :: FilePath -> IO String~ can load the data
    from a file (as specified by the path) into a ~String~,
  2. that the function ~putStrLn :: String -> IO ()~ can print a
    ~String~ to standard output, and
  3. that the function ~getLine :: IO String~ that can read a ~String~
    from standard input.
