...
8. Given the type:
  ...
  differs in behaviour from the error accumulation instance we defined previously.
    ...
    #+BEGIN_SRC haskell :solution
    instance Monoid m => Monad (Error m) where
      return = OK
      (OK a) >>= f = f a
      (Error m) >>= _ = Error m
...
10. (Challenging) Given the types
  ...
  We see that code written in the ~Cont r~ monad is equivalent to code in "continuation passing style", i.e. code where in place of functions of type ~A -> B~ we instead write functions of type ~(B -> r) -> (A -> r)~ that operates in reverse on "continuations".
